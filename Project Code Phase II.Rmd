...
liver_cancer_cell_lines_mutations = prism.snv[which(prism.snv$DepMap_ID %in% liver_cancer_cell_lines),]

```

```{r}
liver_cancer_cell_lines_mutations
```
...
```{r}
save("liver_cancer_cell_lines_mutations", file="All mutations in liver cancer.RData")
```
...
```{r}
sum(liver_cancer_cell_lines_total_mutation_numbers)
save(liver_cancer_cell_lines_total_mutation_numbers, file="liver_cancer_cell_lines_total_mutation_numbers.RData")
```
...
unique(liver_cancer_cell_lines_mutations$DepMap_ID[1:liver_cancer_cell_lines_total_mutation_numbers[1]])
```
```{r}
...
```{r}
#liver_cancer_cell_lines_mutations
#which(table(liver_cancer_cell_lines_mutations$DepMap_ID)>0)
#table(liver_cancer_cell_lines_mutations$DepMap_ID)[which(table(liver_cancer_cell_lines_mutations$DepMap_ID)>0)]
sum(table(liver_cancer_cell_lines_mutations$DepMap_ID)[which(table(liver_cancer_cell_lines_mutations$DepMap_ID)>0)])
dim(liver_cancer_cell_lines_mutations)
```
...
liver_mutated_genes
#sum(liver_mutated_genes) is 8482
#length(liver_mutated_genes) this is 487
```

```{r}
save(liver_mutated_genes, file="most_frequently_mutated_genes_in_liver_cancer.RData")
save(mutated_genes, file="most_frequently_mutated_genes.RData")
```
...
t.test(mean_values_tp53, mu=mean_all_wo_tp53)
...
```{r}
#check for B2M mutation, because it prevents metastasis
#which(prism.snv$Hugo_Symbol == "B2M")
b2m_cells_mutation = unique(prism.snv$DepMap_ID[which(prism.snv$Hugo_Symbol == "B2M")])
#prism[prism.snv$DepMap_ID[which(prism.snv$Hugo_Symbol == "B2M")],]
prism[which(rownames(prism) %in% b2m_cells_mutation),]

```
```{r}
apply(prism[which(rownames(prism) %in% b2m_cells_mutation),],1,mean,na.rm=TRUE)
mean(apply(prism[which(rownames(prism) %in% b2m_cells_mutation),],1,mean,na.rm=TRUE),na.rm=TRUE)
#Nice! We see, that all prism scores are negative for this mutation! Let's compare this to all cell lines
```
```{r}
apply(prism,1,mean,na.rm=TRUE)
mean(apply(prism,1,mean,na.rm=TRUE),na.rm=TRUE)
#ok, so this doesn't really tell us anything
```
```{r}
new_prism.exp
```
...
```{r}
#cor_of_prism.exp
#which(is.na(cor_of_prism.exp))
```
```{r}
cor_of_prism.achilles
```
```{r}
correlation_matrix_exp_ach_dataframe = as.data.frame(correlation_matrix_exp_ach, use = "pairwise.complete.obs")
#correlation_matrix_exp_ach_dataframe = as.data.frame(correlation_matrix_exp_ach)
```
```{r}
1+1
```
...
```{r}
head(correlation_matrix_exp_ach_dataframe)
#ok it gives me NAs, check later
#histogramm, check later
```
