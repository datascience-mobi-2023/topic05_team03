...
for (i in 1:length(all_drugs_grouped_)) {
  ...
  cell_growth_rates <- as.matrix(prism_for_liver[, all_drug_names])
...
```{r}
# İlaç gruplarını döngü içinde ele almak için indeksler oluşturun
all_drugs_grouped_ <- split.default(prism, rep(1:(ncol(prism)/8), each = 8))
...
for (i in 1:length(all_drugs_grouped_)) {
  ...
  cat("Drug group:", names(all_drugs_grouped_)[i], "\n")
  
  # Linear regresyon analizini yapın
  for (j in 1:8) {
    # İlgili ilaçın hücre büyüme değerlerine erişin
    cell_growth_rates <- drug_group[, j]
    ...
    cat("Drug:", colnames(drug_group)[j], "\n")
