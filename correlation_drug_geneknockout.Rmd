...
```{r}
#which(isTRUE(as.character(prism.cl$disease == "Liver Cancer")))
liver_cancer_indices = which(prism.cl$disease == "Liver Cancer")
...
```{r}
same_targets = which( #TARGETS OF HIGHLY CORRELATED DRUGS %in% Liver_cancer_target_genes) )
length(same_targets)
same_targetsSSSS

names_highly_correlated_drugs = names(highly_correlated_drugs)
prism_highly_correlated_drugs = prism[,names_highly_correlated_drugs]
...
same_targets = which(names_correlations_above_threshold_geneknockout %in% Liver_cancer_target_genes)
length(same_targets)
same_targets
Liver_cancer_target_genes[same_targets]

#!!!!!!ALL THEM GENES HAVE A PAIRED DRUG--> FIND THEM 
```
