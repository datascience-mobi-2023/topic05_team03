...
lapply(prism, 2, function(x){
  #fist remove all NAs
  x.remove = which(is.na(x))
  x.new.prism = x[-i.remove]
 
   lapply(prism.achilles, 2, function(y){
      #fist remove all NAs
  y.remove = which(is.na(y))
  y.new.prism = y[-i.remove]
  cor(x,y, method = "spearman")
   })#funktion, die über x und alle spalten von achilles corrm berechnet
