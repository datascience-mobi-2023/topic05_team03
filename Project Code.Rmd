...
```
```{r}
save(our_target_genes, file = "relevant genes for cell survival, targets.RData")
save(drugs_scores, file = "drugs that target relevant genes for cell survival and their scores on every cell line.RData")
save(important_drugs_dataframe, file = "mean of values over all cell lines for drugs that target genes important for cell survival.RData")
```
...
```{r}
save(new_prism.treat, file="new_prism.treat.RData")
...
```
```{r}
save(liver_cancer_cell_lines, file="liver_cancer_cell_lines.RData")
```
